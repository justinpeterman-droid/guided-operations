...
| Incident and report workflow    | Protected `/incidents/new` and `/reports` exist. The incident workspace requires explicit category confirmation and a confirm/exclude decision for every conservative note-line fact proposal before save. Fictional local browser qualification covers that front-half review plus scoped confirmed facts, officer review/edit/finalization, correction, bounded stale-conflict recovery without losing typed text, append-only history/restore, audited print, and same-facility administrator access to an officer's report. | Model-backed structured extraction, complete approved gap-to-draft rules, supported explicit-revision export, hosted proof, and owner acceptance.         |
...
The current safe foundation treats each non-empty officer note line as one
unchanged proposal, shows that exact source beside the proposed fact, and
requires the officer to confirm it or exclude it. Editing a proposal resets its
decision and a confirmed edit is retained as a separate officer-review source
note. This is deterministic review scaffolding, not model-backed extraction; the
provider extraction boundary and persisted resumable state remain open.
