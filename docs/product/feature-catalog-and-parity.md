...
| Incident and report workflow    | Protected `/incidents/new` and `/reports` exist. A protected provider-neutral AI route can suggest an allowlisted category and source-line-bound facts; every suggestion remains pending until explicit officer confirmation, and unavailable/invalid AI falls back to manual line review. Fictional local qualification also covers scoped confirmed facts, officer review/edit/finalization, correction, bounded stale-conflict recovery without losing typed text, append-only history/restore, audited print, deterministic explicit-version reviewed-report Word download, and same-facility administrator access to an officer's report. Copy-only report types have no print/download action. | Pinned-model extraction evaluation with approved provider data controls, complete approved gap-to-draft rules, approved 005/409 source-form output and fidelity, hosted proof, and owner acceptance.             |
| Forms and packets               | Protected `/forms` exposes the reviewed Count Sheet with its actual capabilities, routes administrators to the protected six-form Daily Paperwork workspace, denies that route-level action to officers, labels Chain of Custody as an official physical-only process with no digital action, and keeps unapproved Monthly work unavailable. The fictional visual Preview remains separate.                                                                                                                                                                                                                                                                                                          | Approved digital-form sources, search/filter if the approved catalog grows enough to require it, eligible add-to-incident and packet workflows, form fidelity, records disposition, and signed-in browser proof. |
...
| Accounts and staff              | Protected `/admin/accounts` supports invitation with approved shift assignment, roster shift visibility/change, reset, unlock, disablement, and role change with purpose-bound step-up, session revocation, lifecycle guards, redacted audit metadata, and content-free outcome/correlation events that exclude administrator/target IDs, employee data, proof tokens, and temporary passcodes.                                                                                                                                                                                                                                                                                                      | Hosted Auth/revocation proof, fictional administrator browser flow, credential-delivery procedure, responsive/accessibility qualification, acceptance.                                                           |
| Audit and system health         | Protected `/admin/audit` and `/admin/health`, strict runtime readiness, and allowlisted sign-in, sign-out, passcode-change, policy, report, incident-fact, Daily Paperwork package, administrator step-up, account-lifecycle, legal-hold, and retention-deletion operational events exist. Authentication, step-up, account, and retention lifecycle responses carry an opaque request ID without account, session, credential, employee, target, administrator, passcode, proof-token, authority, backup, role, or shift fields.                                                                                                                                                                    | Hosted sinks/access/retention, Storage/DB signals, dashboards, alerts, test delivery, budgets, and observation.                                                                                                  |
...
| Forms Library            | `/workspace/forms`; **SOURCE-PARTIAL**                                                                     | `/forms`                          | **PROTECTED CAPABILITY CATALOG; MIGRATION-BACKLOG** | Protected catalog exposes the Count Sheet's implemented open/history/print capabilities, a role-gated Daily Paperwork entrance, explicit Chain of Custody obtain-and-hand-complete guidance with no digital substitute, and unavailable Monthly packets. Add source/revision details, search/filter when warranted by approved catalog size, multi-select packet preview, and authorized add-to-incident as eligible approved forms arrive. Old source does not prove complete document rendering.                                                                          |
...
| Daily Paperwork          | Admin Paperwork Center; **SOURCE-IMPLEMENTED**                                                             | `/admin/paperwork/daily`          | **PROTECTED GENERIC WORKFLOW; SOURCES OPEN**        | Administrator-only six-type catalog and generic private open/edit/save/history/exact-restore/audited-print workflow exist locally, including server-owned bounded scalar/repeating-row schemas, immutable source binding, stale-session and stale-save denial, idempotency, retirement handling, and negative database tests. No private source body is committed. Import and map the six approved definitions, prove source-order screen/print fidelity, complete responsive/accessibility/browser qualification, apply the hosted migration, and obtain owner acceptance. |
...
The current safe foundation includes a protected provider-neutral extraction
boundary. It can suggest only an allowlisted category and facts tied to actual
source-line keys; server validation restores the exact source text shown beside
each proposal. Every suggestion still requires confirm/exclude review, and an
edit resets confirmation. A confirmed rewrite retains both the original field
note and a separate officer-review note as provenance. Disabled, unavailable, or
invalid AI leaves the deterministic one-line/one-proposal manual path available.
Pinned-model evaluation, approved provider controls for operational content, and
persisted resumable state remain open.
