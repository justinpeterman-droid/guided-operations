...
| NCU Days Count                  | Exact reviewed structure, protected assigned-shift load/save/reopen UI, append-only revisions, stale-write response, database form enforcement, and fictional Chromium calculation proof exist.                                 | Revision-history UI, conflict recovery acceptance, one-page print/export fidelity, accessibility breadth, hosted proof, and owner acceptance.            |
...
| Accounts and staff              | Protected `/admin/accounts` supports invitation with approved shift assignment, roster shift visibility, reset, unlock, disablement, and role change with purpose-bound step-up, lifecycle guards, and redacted audit metadata. | Shift-change workflow for existing staff, hosted Auth/revocation proof, fictional administrator browser flow, credential-delivery procedure, acceptance. |
...
| Forms Library            | `/workspace/forms`; **SOURCE-PARTIAL**                                                                     | `/forms`                          | **MIGRATION-BACKLOG**                         | Search/filter approved catalog, capabilities, source/revision, physical-only guidance, multi-select packet preview, and authorized add-to-incident. Old source exposes planning/eligibility for some actions and does not prove complete document rendering.                                                                                                                           |
| NCU Days Count           | `/workspace/count-sheet`; **SOURCE-IMPLEMENTED**                                                           | `/count-sheet`                    | **FOUNDATION slice only; MIGRATION-BACKLOG**  | The new repo has calculation/schema/types/tests, not a user-facing feature. Preserve source row order, browser entry, reconciliation, explicit mismatch, dated/shift record, revision-safe saves, one-page letter landscape print, blank-vs-zero rules, keyboard/mobile use, and audit of deliberate output actions.                                                                   |
...
| Daily Paperwork          | Admin Paperwork Center; **SOURCE-IMPLEMENTED**                                                             | `/admin/paperwork/daily`          | **MIGRATION-BACKLOG**                         | Six source workflows: Assignment Roster, Metal Detector Test, Perimeter Check, Random Search Log, Detector Sign-Out, Uniform Inspection. Preserve record date/shift, optimistic revision, autosave truth, conflict recovery, source-order print, and accessible mobile editing. Validate every field/rule against approved source material.                                            |
