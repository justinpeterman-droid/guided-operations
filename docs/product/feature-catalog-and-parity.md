...
| Incident and report workflow    | Protected `/incidents/new` and `/reports` exist. Fictional local browser qualification covers scoped confirmed facts, officer review/edit/finalization, correction, bounded stale-conflict recovery without losing typed text, append-only history/restore, audited print, and same-facility administrator access to an officer's report. | Full six-step incident creation/extraction/gap-to-draft parity, supported explicit-revision export, hosted proof, and owner acceptance.                   |
