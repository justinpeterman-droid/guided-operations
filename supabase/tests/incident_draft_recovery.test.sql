...
select is((select outcome from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',0,1::smallint,'{"schemaVersion":1}',null,null,repeat('a',64))),'saved','first save succeeds');
select is((select revision_number from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',0,1::smallint,'{"schemaVersion":1}',null,null,repeat('a',64))),1,'lost first response retries without a duplicate');
...
select is((select outcome from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',0,1::smallint,'{"schemaVersion":1,"notes":"changed"}',null,null,repeat('b',64))),'conflict','changed create retry cannot overwrite');
select is((select revision_number from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',1,1::smallint,'{"schemaVersion":1,"notes":"changed"}',null,null,repeat('b',64))),2,'revision update succeeds');
select is((select revision_number from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',1,1::smallint,'{"schemaVersion":1,"notes":"changed"}',null,null,repeat('b',64))),2,'lost update response retries safely');
select is((select outcome from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',1,1::smallint,'{}',null,null,repeat('c',64))),'conflict','stale update rejected');
...
select is((select outcome from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',2,1::smallint,'{}',null,null,repeat('c',64))),'conflict','other owner cannot save');
...
select is((select count(*)::int from api.save_incident_draft('a3000000-0000-4000-8000-000000000001',2,1::smallint,'{}',null,null,repeat('c',64))),0,'revoked session cannot save');
...
set local role authenticated;
select is((select outcome from api.save_incident_draft('a3000000-0000-4000-8000-000000000003',0,1::smallint,'{}',null,null,repeat('f',64))),'saved','independent unfinished draft saves');
