...
begin
  ...
  insert into app_private.idempotency_records (
    ...
    auth.uid(), 'report.finalize', p_idempotency_key_digest, p_request_digest,
    statement_timestamp() + interval '24 hours'
  );

  insert into app_private.reports (
    incident_id, report_type, reporting_account_id, prepared_by_account_id
  ) values (
    candidate_record.incident_id, candidate_record.report_type, auth.uid(), auth.uid()
  ...
  insert into app_private.report_revisions (
    ...
    narrative, schema_version, provenance
  ) values (
    report_id, 1, auth.uid(), candidate_record.source_incident_revision_id,
    p_narrative, 1,
    jsonb_build_object('draft_candidate_id', candidate_record.id::text)
  );

  update app_private.idempotency_records as record
    set status = 'succeeded', result_reference_id = report_id,
...
comment on function api.finalize_report_draft_candidate(uuid, text, text, text) is
  'Creates the first immutable report revision only after an authorized human explicitly finalizes a review-only draft candidate.';
