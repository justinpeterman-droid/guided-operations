...
create or replace function app_private.disable_account(
...
begin
  ...
  insert into app_private.audit_events(
    ...
    jsonb_build_object('outcome', 'disabled')
  );
end;
$$;

comment on function app_private.disable_account(uuid, uuid) is
  'Private same-facility account disablement. The lifecycle trigger enforces last-active-administrator protection and session-version revocation.';

revoke all on function app_private.disable_account(uuid, uuid)
