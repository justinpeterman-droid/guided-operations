...
import { recordReportPrintForCurrentSession } from "./record-report-print";
...
describe("recordReportPrintForCurrentSession", () => {
  it("hashes retry material and records only an opaque report print request", async () => {
    const current = client({
      data: "66666666-6666-4666-8666-666666666666",
      error: null,
    });
    await expect(
      recordReportPrintForCurrentSession(
        command,
        current as never,
        "k".repeat(32),
      ),
  ...
  it("keeps request correlation out of the retry identity", async () => {
    ...
    await recordReportPrintForCurrentSession(
      command,
      first as never,
      "k".repeat(32),
    );
