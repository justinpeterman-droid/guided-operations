...
type RestoreCountSheetClient = CurrentSessionClient &
  Readonly<{
    rpc(
      name: "restore_count_sheet_revision",
      args: Readonly<{
        p_record_id: string;
...
export async function restoreCountSheetRevisionForCurrentSession(
  candidate: unknown,
  client: RestoreCountSheetClient,
  hmacKey: string,
) {
  ...
  try {
    const result = await client.rpc("restore_count_sheet_revision", {
      ...
      p_idempotency_key_digest: digest(
        ...
        hmacKey,
        "count_sheet.restore.key",
      ),
      p_request_digest: digest(
        JSON.stringify(request),
        hmacKey,
        "count_sheet.restore.request",
      ),
    });
