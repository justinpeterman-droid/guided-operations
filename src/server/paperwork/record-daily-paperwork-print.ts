...
type RecordDailyPaperworkPrintClient = CurrentSessionClient &
  Readonly<{
    rpc(
      name: "record_daily_paperwork_print_v2",
      args: Readonly<{
        p_record_id: string;
...
export async function recordDailyPaperworkPrintForCurrentSession(
  candidate: unknown,
  client: RecordDailyPaperworkPrintClient,
  hmacKey: string,
) {
  ...
  try {
    const result = await client.rpc("record_daily_paperwork_print_v2", {
      ...
      p_idempotency_key_digest: digest(
        ...
        hmacKey,
        "daily_paperwork.print.key",
      ),
      p_request_digest: digest(
        JSON.stringify(request),
        hmacKey,
        "daily_paperwork.print.request",
      ),
