...
function sameKeys(
  ...
  return (
    keys.length === expected.length && expected.every((key) => key in actual)
  );
}
...
export function validateCountPayload(
  ...
  for (const area of structure.areas) {
    ...
    if (!row || !sameKeys(row, structure.columns)) {
      throw new Error(
        "Count Sheet values do not match the approved structure.",
      );
    }
    for (const column of structure.columns) {
      if (!validCount(row[column])) {
        throw new Error(
          "Count Sheet values must be nonnegative whole numbers.",
        );
      }
    }
  }
