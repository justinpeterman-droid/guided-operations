...
def normalize_extraction(
    ...
    if extraction.observed_page_count is not None:
        ...
        page_count = extraction.page_count or (
            max(by_page) + 1 if by_page else 0
        )
    invalid_page_indexes = sorted(
        page_index
    ...
    if invalid_page_indexes:
        raise ValueError(
            "Extraction contains blocks outside the observed page range"
        )
    ...
    for zero_based_index in range(page_count):
        ...
        for block in page_blocks:
            ...
            if not text:
                continue
            is_heading = (
                block.kind in {"title", "heading", "section_header"}
        ...
        pages.append(
            NormalizedPage(
                ...
                section_path=(
                    " > ".join(heading_stack) if heading_stack else None
                ),
                ocr_confidence=ocr_confidence,
                quality_flags=tuple(sorted(quality_flags)),
                warning_codes=tuple(sorted(warnings)),
                extraction_warning=(
                    "; ".join(sorted(warnings))[:500] if warnings else None
                ),
